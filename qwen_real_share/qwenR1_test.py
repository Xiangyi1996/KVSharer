...
class Qwen2Attention(nn.Module):
    ...
    def forward(
            ...
            kv_cache_share_layers_map: Optional[Dict] = None,  # 添加KV缓存共享参数
        ...
        if past_key_value is not None:
            ...
            if kv_cache_share_layers_map is None:
                ...
                if kv_usage_layer_ids == self.layer_idx:
                    ...
                    # 当前层共享其他层的KV缓存
                    # 第一步：仍然更新当前层的缓存（其他层可能需要）
                    past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
                    
                    # 第二步：获取要使用的共享层的KV缓存
                    if (len(past_key_value.key_cache) > kv_usage_layer_ids and 
                        len(past_key_value.key_cache[kv_usage_layer_ids]) > 0):
                        key_states = past_key_value.key_cache[kv_usage_layer_ids]
                        value_states = past_key_value.value_cache[kv_usage_layer_ids]
                    else:
                        # 共享层还没有缓存，使用当前层的
    ...
    def forward(
        ...
        if past_key_value is not None:
            if kv_cache_share_layers_map is None:
                ...
                # if skip:
                #     print("==Skip is True on sdpa==")
                #     print("kv_usage_layer_ids:", kv_usage_layer_ids)
                #     print("self.layer_idx:", self.layer_idx)
                #     print('=======================')
                cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                key_states, value_states = past_key_value.update(key_states, value_states, kv_usage_layer_ids,
                                                                 cache_kwargs)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        # print('kv_cache_share_layers_map_qwenR1model', kv_cache_share_layers_map)
