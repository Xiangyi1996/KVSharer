...
class DynamicDictCache(Cache):
    ...
    def update(
        ...
        KVSharer修复：移除skip参数，总是更新缓存，因为其他层可能需要当前层的KV值
        ...
        if kv_cache_share_layers_map is None:
            ...
            for layer_idx in range(len(kv_cache_share_layers_map.keys())):
                if kv_cache_share_layers_map[layer_idx] == layer_idx:
                    ...
                    legacy_cache += ((self.key_cache[kv_cache_share_layers_map[layer_idx]],
                                      self.value_cache[kv_cache_share_layers_map[layer_idx]]),)
                else:
                    # print('add None')
