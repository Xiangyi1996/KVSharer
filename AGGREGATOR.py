...
from E2E.run.utils import common_kwargs, load_dataset_local
...
class ModelInference:
    def __init__(self, args, device: str = "cuda"):
        ...
        self.kvsharer_enabled = self.strategy in ["kvsharer", "fusion"]
        self.adaskip_enabled = self.strategy in ["adaskip", "fusion"]
    ...
    def _load_model_with_adaskip(self):
        """加载支持AdaSkip的模型"""
        try:
            ...
            if self.model_name == 'LlamaR1':
                ...
                self.model = LlamaForCausalLM.from_pretrained(model_id, config=config, **common_kwargs).to(self.device)
            elif self.model_name == 'QwenR1':
                config = MyQwenR1Config.from_pretrained(model_id, SKIP_SUB_LAYER_NUM=self.skip_sub_layer_num)
                self.model = Qwen2ForCausalLM.from_pretrained(model_id, config=config, **common_kwargs).to(self.device)
            else:
            ...
            # 如果是融合策略，检查KVSharer兼容性
            if self.kvsharer_enabled:
                logger.warning("⚠️  融合策略：AdaSkip模型可能不支持KVSharer，需要确认模型兼容性")
    ...
    def load_model_and_tokenizer(self):
        try:
            ...
            path = agent_map.get(self.model_name)
            tokenizer = AutoTokenizer.from_pretrained(path)
            
            # 使用自定义的MyLlamaConfig，传入SKIP_SUB_LAYER_NUM参数
            if self.model_name == 'LlamaR1':
                config = MyLlamaConfig.from_pretrained(path, SKIP_SUB_LAYER_NUM=self.skip_sub_layer_num)
                model = LlamaForCausalLM.from_pretrained(path, config=config, **common_kwargs).to(self.device)
            elif self.model_name == 'QwenR1':
                config = MyQwenR1Config.from_pretrained(path, SKIP_SUB_LAYER_NUM=self.skip_sub_layer_num)
                model = Qwen2ForCausalLM.from_pretrained(path, config=config, **common_kwargs).to(self.device)
            else:
                raise ValueError(f"Unsupported model for AdaSkip: {self.model_name}")
    ...
    def generate_response_adaskip(self, agg_prompts, ):
        ...
        # Warmup 预填充阶段的离线重要性学习：通过预热阶段收集子层重要性信息
        ...
        for prompt in tqdm(warmup_data_all):
            ...
            input = tokenizer(prompt, truncation=False, return_tensors="pt").to(self.device)
        ...
        # Target 解码阶段的在线重要性学习：根据当前序列动态调整跳层策略
        ...
        for prompt in tqdm(dataloader, desc="Evaluating"):
            ...
            # 生成输出（根据数据集类型调整生成参数）
            # 使用预热阶段收集的信息来决定哪些层可以跳过
            # 对于每个样本，生成模型输出
            # 特殊处理samsum数据集，防止模型无限重复输出
            # 保存预测结果用于后续评估
            ...
            kv_cache_memory = self.calculate_kvcache_memory(model, context_length, output_length, batch_size=1)
    ...
    def generate_response(
        ...
        # 注意：AdaSkip策略不使用此方法，而是直接使用_generate_with_adaskip
        # 这是因为AdaSkip需要特殊的批处理和预热流程
        
        if self.strategy == "fusion":
            # 融合策略（目前暂不支持，因为模型架构差异）
            logger.warning("融合策略暂未实现，回退到baseline策略")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--skip_sub_layer_num', type=int, default=8, 
                        help='AdaSkip跳过的子层数量')
    parser.add_argument('--max_gen', type=int, default=10240, 
                        help='AdaSkip最大生成长度')
