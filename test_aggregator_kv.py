...
from agent import *
from llama_real_share.modeling_llama_kvsharer_updated import LlamaForCausalLM
...
class KVCacheManager:
    ...
    def calibrate(self, max_length: int = 64):
        """校准阶段：捕获每层KV缓存"""
        logger.info("开始校准阶段，捕获KV缓存...")
        self.kv_cache_share_layers_map = {i: i for i in range(self.num_layers)}
        
        with torch.no_grad():
            for text in tqdm(self.calibration_set, desc="校准KV缓存"):
    ...
    def build_sharing_strategy_1(self) -> Dict[int, int]:
        """构建KV共享策略"""
        logger.info("构建KV共享策略...")
        distance_matrix = self.analyze_kv_similarity()
        num_layers = distance_matrix.shape[0]
        
        # 生成层对组合（按相似度排序）
        layer_pairs = [
            (i, j) for i in range(num_layers)
            for j in range(i + 1, num_layers)
        ]
        layer_pairs.sort(key=lambda x: distance_matrix[x[0]][x[1]], reverse=True)
        
        # 初始化共享映射
        shared_layers = set()
        self.kv_cache_share_layers_map = {i: i for i in range(num_layers)}
        
        # 构建共享策略
        for pair in layer_pairs:
            if len(shared_layers) >= self.max_shared_layers:
                break
            
            if pair[0] in shared_layers:
                continue
            
            # 创建临时映射
            tmp_map = deepcopy(self.kv_cache_share_layers_map)
            tmp_map[pair[0]] = pair[1]
            
            # 验证相似性
            sim_value = self.validate_sharing(tmp_map)
            
            if sim_value > self.threshold:
                self.kv_cache_share_layers_map = tmp_map
                shared_layers.add(pair[0])
                logger.info(f"层 {pair[0]} 共享至层 {pair[1]}，相似度: {sim_value:.4f}")
                
        print(self.kv_cache_share_layers_map)
        
        return self.kv_cache_share_layers_map
    ...
    def build_sharing_strategy(self, threshold, max_shared_layers) -> Dict[int, int]:
        ...
        for pair in tqdm(pos_rank):
            tmp_kv_cache_share_layers_map = deepcopy(self.kv_cache_share_layers_map)
            ...
            if sim_value > threshold:
                self.kv_cache_share_layers_map = deepcopy(tmp_kv_cache_share_layers_map)
    ...
    def validate_sharing(self, test_map: Dict) -> float:
        """验证共享策略对输出的影响"""
        sim_values = []
        for text in self.calibration_set[:5]:  # 使用部分数据验证
            ...
            with torch.no_grad():
                outputs2 = self.model(
                    **inputs,
                    output_hidden_states=True,
                    kv_cache_share_layers_map=test_map
                )
...
class ModelInference:
    def __init__(self, model_name: str, download_dir: str, use_kvsharer: bool = True, device: str = "cuda"):
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            sharing_strategy = self.kv_manager.build_sharing_strategy(self.threshold, self.max_shared_layers)
    ...
    def generate_response_with_sharing(
            self,
            prompt: str,
            batch_size: int,
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                kv_cache_share_layers_map=self.kv_manager.kv_cache_share_layers_map,
...
class ExperimentRunner:
    def __init__(self, args: argparse.Namespace):
        self.args = args
        self.model_tester = ModelInference(args.aggregator, args.download_dir, args.use_kvsharer)
        self.results = []
        self.num_correct = 0
    ...
    def run_evaluation(self) -> None:
        ...
        for sample in tqdm(dataloader, desc="Evaluating"):
            ...
            if self.args.use_kvsharer:
                response = self.model_tester.generate_response_with_sharing(sample, self.args.batch_size)
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--use_kvsharer", type=bool, default=True)
